...
INCREASE_BEF_BUY = 0.001
INCREASE_BEF_SELL = 0.001
DECREASE_STOP_LOSS = 0.001
EWM_SMOOTH_SPAN = 5
...
while True:
  ...
  if position == 'pending_buy':
      ...
      if lastBuyOrder.status == "filled" or lastBuyOrder.status == "partially_filled":
        buy_qty = lastBuyOrder.filled_qty
        buy_value = lastBuyOrder.filled_avg_price
        data_string = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' : buy value = ' + str(buy_value) + ' buy amount = ' + str(buy_amount)
  ...
  if position == 'sell':
    ...
    if bool(res[0]):
      order = tradeSellLimit(ASSET, buy_qty, current_val)
  ...
  if position == 'pending_sell':
      ...
      if lastSellOrder.filled_at != None:
        sell_tot_qty = lastSellOrder.qty
        sell_qty = lastSellOrder.filled_qty
        sell_value = lastSellOrder.filled_avg_price
        profit *= sell_value/buy_value
        print(profit)
        data_string = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' : sell value = ' + str(sell_value) + ' sell amount = ' + str(sell_amount) + '\n' + str(datetime.now().time()) + ' : *** profit = ' + str(profit)
        writeData(data_string, profit)
