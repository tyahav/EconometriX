...
def tradeBuyLimit(asset,buy_amount, buy_price):
  tss = str(int(time.time()*1000))
  q = []
  q.append("symbol="+asset)
  q.append("side=BUY")
...
def tradeSellLimit(asset,base_qty, sell_price):
  ...
  q.append("type=LIMIT")
  q.append("timeInForce=GTC")
  q.append("quantity="+"{:.8f}".format(int(base_qty*100000000)/100000000))
...
# Trade parameters
ASSET = 'NONE'
BASE_ASSET_LIST = ['BTC', 'ETH', 'BNB', 'ADA', 'DOGE', 'XRP', 'BCH', 'LTC', 'LINK','THETA','XLM']
QUOTE_ASSET='USDT'
...
while True:
  ...
  if position == 'sell':
    ...
    if res[0]:
      ...
      #max_sell_value_allowed = float(getFilterValue(ASSET,'MARKET_LOT_SIZE')['maxQty'])
